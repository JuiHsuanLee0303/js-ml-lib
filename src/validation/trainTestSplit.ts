 ...
 * @returns {{train: T[], test: T[]}} Object containing training and testing sets
 */
export function trainTestSplit<T>(
  ...
  if (!data || data.length === 0) {
    throw new DataValidationError("Input data cannot be empty");
  }
  if (testSize <= 0 || testSize >= 1) {
    throw new DataValidationError("Test size must be between 0 and 1");
  }
  ...
  if (testCount === 0) {
    throw new DataValidationError(
      "Test size too small, resulting in empty test set"
    );
  }
